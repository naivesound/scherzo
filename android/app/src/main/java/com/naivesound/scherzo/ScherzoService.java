...
//        mScherzo = new Scherzo(44100);
    ...
    public void handleMIDI() {
        final MidiManager midiManager = (MidiManager)getApplicationContext().getSystemService(MIDI_SERVICE);
...
//            }
    ...
    @TargetApi(Build.VERSION_CODES.M)
    private void openMIDIDevice(MidiManager midiManager, MidiDeviceInfo device) {
                        ...
                        for (int i = 0; i < device.getInfo().getOutputPortCount(); i++) {
                            ...
                            outputPort.connect(new MidiReceiver() {
                                public void onSend(byte[] msg, int offset,
                                    ...
                                    while (count >= 3) {
                                        Log.d("MIDI", "" + msg[offset] + " " + msg[offset+1] + " " + msg[offset+2]);
                                        mScherzo.midi(msg[offset], msg[offset+1], msg[offset+2]);
                                        offset += 3;
                                        count -= 3;
                                    }
                                }
    ...
    public void noteOff(int chan, int note) {
    ...
    public void looperCommand(boolean primary) {
        mScherzo.looperCommand(primary);
    }

    public class ScherzoBinder extends Binder {
        ScherzoService getService() {
            return ScherzoService.this;
        }
    }
}
