...
int fluid_rampreset_preset_noteon(fluid_preset_t *preset, fluid_synth_t *synth,
 ...
 */
char *fluid_ramsfont_get_name(fluid_ramsfont_t *sfont) { return sfont->name; }
 ...
 */
int fluid_ramsfont_add_preset(fluid_ramsfont_t *sfont,
                              fluid_rampreset_t *preset) {
...
/* Note for version 2.0 : missing API fluid_ramsfont_izone_get_gen - Antoine
 * Schmitt May 2003 */
...
/* Note for version 2.0 : missing API fluid_ramsfont_izone_get_loop - Antoine
 * Schmitt May 2003 */
...
int fluid_rampreset_get_num(fluid_rampreset_t *preset) { return preset->num; }
 ...
 */
int fluid_rampreset_add_zone(fluid_rampreset_t *preset,
                             fluid_preset_zone_t *zone) {
  if (preset->zone == NULL) {
    zone->next = NULL;
    preset->zone = zone;
 ...
 */
int fluid_rampreset_remembervoice(fluid_rampreset_t *preset,
                                  fluid_voice_t *voice) {
...
void fluid_rampreset_updatevoices(fluid_rampreset_t *preset, int gen_type,
  ...
  /* walk the presetvoice to update them if they are still active and ours.
     If their ID has changed or their state is not playing, they are not ours,
     so we forget them
  */
  while (tmp) {
    fluid_rampreset_voice_t *presetvoice =
 ...
 */
int fluid_rampreset_noteon(fluid_rampreset_t *preset, fluid_synth_t *synth,
                           int chan, int key, int vel) {
