...
struct _fluid_synth_t {
  ...
  uint32_t noteid; /** the id is incremented for every new note. it's used for
                      noteoff's  */
  uint32_t storeid;
  int nbuf; /** How many audio buffers are used? (depends on nr of audio
               channels / groups)*/
  ...
  fluid_mutex_t busy; /* Indicates, whether the audio thread is currently
                       * running.
                       * Note: This simple scheme does -not- provide 100 %
                       * protection against
                       * thread problems, for example from MIDI thread and shell
                       * thread
                       */
};
...
int fluid_synth_get_chorus_type(
    fluid_synth_t *synth); /* see fluid_chorus_mod */
...
enum fluid_interp {
  ...
  /* Fourth-order interpolation: Requires 50 % of the whole DSP processing time,
   * good quality
   * Default. */
...
int fluid_synth_create_key_tuning(fluid_synth_t *synth, int tuning_bank,
                                  int tuning_prog, char *name, double *pitch);
 ...
 * pointer starts
 *  \param lincr The increment, in samples, of the writing pointer in the left
 * buffer
 *  \param rout The sample buffer for the right channel
 *  \param roff The offset, in samples, in the right buffer where the writing
 * pointer starts
 *  \param rincr The increment, in samples, of the writing pointer in the right
 * buffer
...
/** Allocate a synthesis voice. This function is called by a
   ...
   (velocity-to-attenuation,
    velocity to filter, ...)
    Note: A single noteon event may create any number of voices, when the preset
   is layered.
    Typically 1 (mono) or 2 (stereo).*/
fluid_voice_t *fluid_synth_alloc_voice(fluid_synth_t *synth,
                                       fluid_sample_t *sample, int channum,
                                       int key, int vel);
