 ...
 */
int delete_fluid_midi_router_rule(fluid_midi_router_rule_t *rule) {
  FLUID_FREE(rule);
  return FLUID_OK;
}
...
int fluid_midi_router_create_default_rules(fluid_midi_router_t *router) {
  ...
  for (i = 0; i < 6; i++) {
     ...
     */
    if (fluid_midi_router_begin(router, rules[i]) != FLUID_OK)
      goto error_recovery;
 ...
 * Stores the destination (the linked list into which the new rule is inserted
 * by _end)
 */
int fluid_midi_router_begin(fluid_midi_router_t *router,
...
int fluid_midi_router_handle_midi_event(void *data, fluid_midi_event_t *event) {
  ...
  int negative_event =
      0; /* Flag, indicates that current event releases a key / pedal */
  ...
  while (rule) {
    ...
    /* Store the pointer to the next rule right now. If the rule is later
     * flagged for destruction,
     * it may not be accessed anymore.
     */
    next_rule = rule->next;

    /* Check, whether the rule is still active. Expired rules cannot be removed
     * immediately,
     * because freeing memory in a realtime thread is no good idea. And the MIDI
     * thread,
	...
	/* Inverted rule: Exclude everything between max and min (but not
	 * min/max) */
     ...
     * par1 (maybe par2).
     * We keep track on the state of noteon and sustain pedal events. If the
     * application tries
     * to delete a rule, it will only be fully removed, if pending noteoff /
     * pedal off events have
     * arrived. In the meantime (MIDIRULE_WAITING) state, it will only let
