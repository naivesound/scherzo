 ...
 * fluid_getlength
 */
long fluid_getlength(uint8_t *s) {
  long i = 0;
 ...
 * @param player MIDI player instance
 * @param loop Value for looping (DOCME - What would this value be, boolean/time
 * index?)
 * @return Always returns 0
 */
...
fluid_midi_event_t *fluid_midi_parser_parse(fluid_midi_parser_t *parser,
   ...
   * events are sent
   * in-a-row, for example note-ons, then the event type is only sent once. For
   * this case,
   * the last event type (status) is remembered.
   * We simply keep the status as it is, just reset
   * the parameter counter. If another status byte comes in, it will overwrite
   * the status. */
  ...
  case PITCH_BEND:
    ...
    parser->event.param1 =
	((parser->p[1] << 7) | parser->p[0]); /* Note: '|' does here
						      the same as '+' (no
						      common bits), but
						      might be faster */
    break;
 ...
 * Taken from Nagano Daisuke's USB-MIDI driver */
int fluid_midi_event_length(uint8_t event) {
  if (event < 0xf0) {
...
int fluid_midi_send_event(fluid_synth_t *synth, fluid_player_t *player,
  ...
  case NOTE_OFF:
    ...
    break;
  case CONTROL_CHANGE:
    if (fluid_synth_cc(synth, event->channel, event->param1, event->param2) !=
	FLUID_OK) {
      return FLUID_FAILED;
    }
    break;
  case MIDI_SET_TEMPO:
    ...
    break;
  case PROGRAM_CHANGE:
    if (fluid_synth_program_change(synth, event->channel, event->param1) !=
	FLUID_OK) {
      return FLUID_FAILED;
    }
    break;
  case PITCH_BEND:
    if (fluid_synth_pitch_bend(synth, event->channel, event->param1) !=
	FLUID_OK) {
      return FLUID_FAILED;
    }
    break;
