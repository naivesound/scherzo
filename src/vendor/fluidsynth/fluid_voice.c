...
int fluid_voice_init(fluid_voice_t *voice, fluid_sample_t *sample,
  ...
  voice->last_fres =
      -1; /* The filter coefficients have to be calculated later in the DSP
             loop. */
...
int fluid_voice_calc_vol_mod_env(fluid_voice_t *voice) {
  ...
  while (voice->volenv_count >= env_data->count) {
    ...
    // to be the end value of the previous stage
...
#define __pitch(_k) fluid_tuning_get_pitch(tuning, _k)
    ...
    voice->gen[GEN_PITCH].val =
        (voice->gen[GEN_SCALETUNE].val * (voice->key - 60.0f) + 100.0f * 60.0f);
  }
...
void fluid_voice_update_param(fluid_voice_t *voice, int gen) {
  ...
  case GEN_OVERRIDEROOTKEY:
     ...
     * generator
     * can be neglected.
     * NOTE: origpitch sets MIDI root note while pitchadj is a fine tuning
     * amount
  ...
  case GEN_STARTLOOPADDRCOARSEOFS: /* SF2.01 section 8.1.3 # 45 */
    if (voice->sample != NULL) {
      voice->loopstart =
          (voice->sample->loopstart + (int)_GEN(voice, GEN_STARTLOOPADDROFS) +
 ...
 * overwritten
 * mode == FLUID_VOICE_DEFAULT: This is a default modulator, there can be no
 * identical modulator.
 *                             Don't check.
 */
