...
void *fluid_timer_start(void *data) {
  int count = 0;
  int cont = 1;
  long start;
  long delay;
...
fluid_timer_t *new_fluid_timer(int msec, fluid_timer_callback_t callback,
  ...
  if (new_thread) {
    ...
    if (err == 0) {
      FLUID_LOG(FLUID_DBG,
		"The timer thread was created with real-time priority");
    } else {
      /* Create the thread with default attributes */
      err = pthread_create(&timer->thread, NULL, fluid_timer_start,
			   (void *)timer);
      if (err != 0) {
	...
	FLUID_LOG(FLUID_DBG,
		  "The timer thread does not have real-time priority");
      }
    }
  } else {
...
unsigned int fluid_curtime() { return 0; }

#endif

double fluid_utime(void) { return (rdtsc() / fluid_cpu_frequency); }

#if !defined(__i386__)

double rdtsc(void) { return 0.0; }

double fluid_estimate_cpu_frequency(void) { return 1.0; }
...
fluid_profile_data_t fluid_profile_data[] = {
    {FLUID_PROF_WRITE_S16, "fluid_synth_write_s16           ", 1e10, 0.0, 0.0,
     0},
