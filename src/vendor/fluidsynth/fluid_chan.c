 ...
 */
int fluid_channel_get_prognum(fluid_channel_t *chan) { return chan->prognum; }
  ...
  case DATA_ENTRY_MSB: {
    ...
    if (chan->nrpn_active) /* NRPN is active? */
    {
      ...
      if ((chan->cc[NRPN_MSB] == 120) && (chan->cc[NRPN_LSB] < 100)) {
        if (chan->nrpn_select < GEN_LAST) {
          ...
          fluid_synth_set_gen(chan->synth, chan->channum, chan->nrpn_select,
                              val);
        }
    ...
    } else if (chan->cc[RPN_MSB] == 0) /* RPN is active: MSB = 0? */
    {
      ...
      case RPN_PITCH_BEND_RANGE:
        fluid_channel_pitch_wheel_sens(chan,
                                       value); /* Set bend range in semitones */
        /* FIXME - Handle LSB? (Fine bend range in cents) */
        break;
      case RPN_CHANNEL_FINE_TUNE: /* Fine tune is 14 bit over +/-1 semitone (+/-
                                     100 cents, 8192 = center) */
 ...
 */
int fluid_channel_get_num(fluid_channel_t *chan) { return chan->channum; }
