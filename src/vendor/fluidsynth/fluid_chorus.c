...
fluid_chorus_t *new_fluid_chorus(fluid_real_t sample_rate) {
  ...
  for (i = 0; i < INTERPOLATION_SAMPLES; i++) {
	...
	chorus->sinc_table[i][ii] =
	    (fluid_real_t)FLUID_SIN(i_shifted * M_PI) / (M_PI * i_shifted);
	/* Hamming window */
	chorus->sinc_table[i][ii] *=
	    (fluid_real_t)0.5 *
	    (1.0 + FLUID_COS(2.0 * M_PI * i_shifted /
 ...
 * API function, read the current state of the chorus
 */
int fluid_chorus_get_type(fluid_chorus_t *chorus) { return chorus->type; }
...
int fluid_chorus_update(fluid_chorus_t *chorus) {
  ...
  modulation_depth_samples =
      (int)(chorus->new_depth_ms /
	    1000.0 /* convert modulation depth in ms to s*/
	    * chorus->sample_rate);
  ...
  /*  failure: */
  /* Note: This lives on the assumption, that the last chorus values were
   * correct.
...
void fluid_chorus_sine(int *buf, int len, int depth) {
  ...
  for (i = 0; i < len; i++) {
    ...
    buf[i] = (int)((1.0 + val) * (double)depth / 2.0 *
