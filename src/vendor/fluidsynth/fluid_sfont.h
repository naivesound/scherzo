...
typedef struct _fluid_sampledata_t {
  fluid_file fd;
  int16_t *buf;
...
typedef fluid_sampledata_t fluid_sampledata;
...
struct _fluid_sample_t {
  ...
  uint32_t loopstart;
  uint32_t loopend; /* Note: first point following the loop (superimposed on
                       loopstart) */
  uint32_t samplerate;
  int origpitch;
  int pitchadj;
...
#define FLUID_SAMPLETYPE_OGG_VORBIS 16
